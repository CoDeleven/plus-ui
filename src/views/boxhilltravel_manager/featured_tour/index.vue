      ...
      <template #header>
        <div class="toolbar-shell">
          <div class="table-heading">
            <h3>{{ bt('featuredTours') }}</h3>
          </div>
          <div class="toolbar-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['boxhilltravel_manager:featured_tour:add']">{{ bt('addTours') }}</el-button>
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['boxhilltravel_manager:featured_tour:edit']">{{ bt('editSort') }}</el-button>
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['boxhilltravel_manager:featured_tour:remove']">{{ bt('delete') }}</el-button>
...
const confirmAddTours = async () => {
  ...
  if (!selected.length) {
    modal.msgWarning(bt('selectNewFeaturedToursWarning'));
    return;
  }
...
const handleUpdate = async (row?: Partial<Featured_tourVO>) => {
  ...
  showDialog(bt('editFeaturedTour'));
};
...
const handleDelete = async (row?: Partial<Featured_tourVO>) => {
  ...
  await modal.confirm(bt('confirmDeleteFeaturedTour', { ids: _ids }));
