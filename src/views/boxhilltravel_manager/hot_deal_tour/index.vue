<template>
  <div class="p-2 page-shell boxhilltravel-manager-hot_deal_tour-page">
...
<script setup name="Hot_deal_tour" lang="ts">
import {
  addHot_deal_tour,
  delHot_deal_tour,
  getHot_deal_tour,
  listHot_deal_tour,
  updateHot_deal_tour
} from '@/api/boxhilltravel_manager/hot_deal_tour';
...
const hot_deal_tourList = ref<HotDealTourRow[]>([]);
...
const existingHotDealRows = ref<Hot_deal_tourVO[]>([]);
const existingHotDealTourIds = ref<Set<string>>(new Set());
...
const hot_deal_tourFormRef = ref<ElFormInstance>();

const initFormData: Hot_deal_tourForm = {
  id: undefined,
  tourId: undefined,
  sortOrder: undefined
};

const data = reactive<PageData<Hot_deal_tourForm, Hot_deal_tourQuery>>({
  form: { ...initFormData },
  queryParams: {
    pageNum: 1,
    pageSize: 10,
...
const { ids, single, multiple, handleSelectionChange } = useTableSelection<HotDealTourRow>(item => item.id);
const { dialog, resetForm: reset, showDialog, closeDialog } = useFormDialog({
  form,
  formRef: hot_deal_tourFormRef,
...
const loadToursForRows = async (rows: Hot_deal_tourVO[]) => {
  if (!rows.length) {
    return [];
  }
...
/** Query hot deal tour list. */
const getList = async () => {
  await withLoading(async () => {
    const res = await listHot_deal_tour(queryParams.value);
    const rows = res.data?.rows || [];
    hot_deal_tourList.value = await loadToursForRows(rows);
...
const loadExistingHotDealTourIds = async () => {
  const res = await listHot_deal_tour({ pageNum: 1, pageSize: 10000, params: {} });
  const rows = res.data?.rows || [];
  existingHotDealRows.value = rows;
  existingHotDealTourIds.value = new Set(rows.map(item => String(item.tourId)));
};
...
const isTourSelectable = (row: TourVO) => !existingHotDealTourIds.value.has(String(row.id));
...
const handleAdd = async () => {
  ...
  await loadExistingHotDealTourIds();
  await getTourSelectList();
};

const getNextSortOrder = () => {
  const sortOrders = existingHotDealRows.value.map(item => Number(item.sortOrder) || 0);
...
/** Edit sort order. */
const handleUpdate = async (row?: Partial<Hot_deal_tourVO>) => {
  reset();
  const _id = row?.id || ids.value[0];
  const res = await getHot_deal_tour(_id);
...
/** Submit edit. */
const submitForm = () => {
  hot_deal_tourFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
...
.boxhilltravel-manager-hot_deal_tour-page {
